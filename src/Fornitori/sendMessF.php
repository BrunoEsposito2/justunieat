...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $errors .= "Il Testo è obbligatorio e deve avere almeno 2 caratteri <br/>";
            ...
            $orario=time();
            $data=date('d-m-y');
            //$q = "SELECT ID_FORNITORE FROM fornitore WHERE Email ='". $_SESSION["email"] ."'";
            ...
            //$q = "SELECT ID_USER FROM utente WHERE Ristorante='" . $_POST["ristorante"]."'";
            $id_user = $_POST["utente"];
            //$result=mysqli_query($conn, $q);
            //while ($row = mysqli_fetch_array($result, MYSQLI_BOTH)) {
            ...
            $flagUtente = 1;
            $stmt = $conn->prepare("INSERT INTO messaggio (Testo, Data, Orario, ID_USER, ID_RISTORANTE, Titolo, Ricevuto_Dal_Utente) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssiisi", $testo, $data, $orario, $id_user, $id_ristorante, $titolo, $flagUtente);
...
?>
    ...
    <div class="jumbotron" style="background-color:white;">
        ...
        ?>
            ...
            <h3 class="text-center">Messaggio correttamente inviato all'Utente!</h3>
            <form>
